            ...
            <v-card-text v-if="!loading">
                ...
                >
                    <!-- <v-autocomplete  dense :rules="nameRules" item-value="organization_id" item-text="organization_name"  required prepend-inner-icon="mdi-domain" placeholder="Organization"  :items="organizations" v-model="editedObj.organization_id"></v-autocomplete> -->
                     ...
                     <v-text-field :rules="nameRules" required prepend-inner-icon="mdi-account" placeholder="Username" v-model="editedObj.username"></v-text-field>
                    <v-text-field :rules="nameRules" required prepend-inner-icon="mdi-lock" type="password"  placeholder="Password" v-model="editedObj.password"></v-text-field>
                    <v-text-field :rules="nameRules" required prepend-inner-icon="mdi-lock" type="password"  placeholder="Confirm Password" v-model="editedObj.cpassword"></v-text-field>
                   ...
                   <v-card-actions class="justify-end">
                    ...
                    {{ editedObj.method== 0 ? 'Add':"Update" }} Employee</v-btn>
    ...
    methods: {
        async loadPositions() {
            ...
            this.positions = data.filter(rec => { 
                if (this.userInfo.position_id == 1) return rec.position_id == 2 
                else return rec
            })
