        ...
        <v-simple-table dense class="table-container textTitle" color="#BCAAA4">
            <template v-slot:top>
                ...
                <v-toolbar flat dense>
                    ...
                    <v-flex md3>
                        <v-text-field v-model="search" color="#BCAAA4" clearable dense label="Search" append-icon="mdi-magnify" @click:append="searchAccount" @keyup.enter="searchAccount"></v-text-field>
                    </v-flex>
                </v-toolbar>
            </template>
            <thead>
  ...
  components: { AccountsDialog , LoaderView },
    ...
    methods: {
         async resetPassword(item) {
            ...
            await this.classAccount.resetAccountPassword( obj )
            await this.evaluteAccounts()
