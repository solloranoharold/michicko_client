<template>
    <v-container fluid >
            <v-toolbar flat  dense>
                <v-toolbar-title class="toolbarTitle"><v-icon>mdi-monitor</v-icon> Inventory <v-chip v-if="userInfo.position_id==0">{{ $route.params.organization_name}}</v-chip></v-toolbar-title>
                <v-spacer/>
            ...
            <tbody>
                <tr v-for="(item , i) in inventory" :key="i" :style="{'background-color': item.quantity <= 5 ? '#EF9A9A' : 'transparent' }">
                    ...
                    <!-- <td>{{ item.date_created }}</td> -->
                     <td>{{ item.updated_date== "Invalid date" ? "" : item.updated_date }}</td>
                    <td>
                         <v-tooltip bottom >
